...
class BatchREINFORCE:
    ...
    def log_rollout_statistics(self, paths):
        path_returns = [sum(p["rewards"]) for p in paths]
        path_returns_recon = [sum(p['env_infos']["recon_reward"]) for p in paths]
        mean_return = np.mean(path_returns)
        mean_return_recon = np.mean(path_returns_recon)
        std_return = np.std(path_returns)
        min_return = np.amin(path_returns)
        max_return = np.amax(path_returns)
        self.logger.log_kv('stoc_pol_mean', mean_return)
        self.logger.log_kv('stoc_pol_mean_recon', mean_return_recon)
