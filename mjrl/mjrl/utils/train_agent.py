...
def train_agent(job_name, agent,
                ...
                plot_keys = ['stoc_pol_mean', 'stoc_pol_mean_recon'],
                ):
    job_name = os.path.join("job_dir", job_name)
    np.random.seed(seed)
    ...
    for i in range(i_start, niter):
        ...
        if evaluation_rollouts is not None and evaluation_rollouts > 0:
            ...
            mean_pol_perf_recon = np.mean([np.sum(path['env_infos']['recon_reward']) for path in eval_paths])
            if agent.save_logs:
                agent.logger.log_kv('eval_score', mean_pol_perf)
                agent.logger.log_kv('eval_recon_score', mean_pol_perf_recon)
        ...
        if i == 0:
            ...
            result_file.close()
        print("[ %s ] %4i %5.2f %5.2f %5.2f %5.2f " % (timer.asctime(timer.localtime(timer.time())),
                                                 i, train_curve[i], mean_pol_perf, best_perf, mean_pol_perf_recon))
        result_file = open('results.txt', 'a')
        result_file.write("%4i %5.2f %5.2f %5.2f %5.2f \n" % (i, train_curve[i], mean_pol_perf, best_perf, mean_pol_perf_recon))
        result_file.close()
        if agent.save_logs:
