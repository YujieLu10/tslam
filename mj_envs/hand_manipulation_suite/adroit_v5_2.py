...
class AdroitEnv2V5(mujoco_env.MujocoEnv, utils.EzPickle):
    def __init__(self):
    ...
    def step(self, a):
        ...
        for pos in current_pos_list:
            ...
            if pos not in self.previous_contact_points:
                ...
                if min_pos_dist and min_pos_dist > 0.001: 
                    new_points_cnt += 1  
                    reward += 1.2
                elif min_pos_dist and min_pos_dist > 0.0001:
                    new_points_cnt += 1  
                    reward += 0.3
        ...
        if is_touched and self.previous_contact_points != [] and previous_pos_list != []:
            ...
            # try ground truth
            gt_dist1, gt_dist2 = chamfer_dist(torch.FloatTensor([self.obj3_gt]), torch.FloatTensor([self.previous_contact_points]))
            gt_loss = (torch.mean(gt_dist1)) + (torch.mean(gt_dist2))
            reward -= gt_loss
            # TODO: reconstruction bar up
            if gt_loss < 47:
                is_reconstructed = True
        done = False
        # for simplicity goal_achieved depends on the nubmer of touched points
        # TODO: use reconstruction loss
        goal_achieved = is_reconstructed
