...
class AdroitEnv3V4(mujoco_env.MujocoEnv, utils.EzPickle):
    def __init__(self):
    ...
    def step(self, a):
        ...
        # pos cost
        dist = np.linalg.norm(obj_init_pos-palm_pos)
        reward = -dist
        ...
        for contact in self.data.contact:
            if self.sim.model.geom_id2name(contact.geom1) in [self.obj_name[self.obj_bid_idx]] or self.sim.model.geom_id2name(contact.geom2) in [self.obj_name[self.obj_bid_idx]]:#["handle", "neck", "head"]:
                ...
                # is_touched reward
                is_touched = True
        # exp5 is_touched reward -> new contact points reward plus new contact points distance constraint
        # if is_touched:
        #     reward += 0.01
        ...
        for pos in current_pos_list:
            if pos not in self.previous_contact_points:
                ...
                if min_pos_dist != 10:
                    if min_pos_dist > 0.001: 
                        # print(min_pos_dist)
                        new_points_cnt += 1  
                        reward += 1.2
                    elif min_pos_dist > 0.0001:
                        new_points_cnt += 1  
                        reward += 0.3
    ...
    def get_obs(self):
        ...
        # obj_init_pos = self.data.body_xpos[self.obj_bid_list[self.obj_bid_idx]].ravel()
        # palm_pos = self.data.site_xpos[self.S_grasp_sid].ravel()
        # return np.concatenate([qp[:-6], palm_pos, palm_pos-obj_init_pos, [len(self.previous_contact_points)]])
        qp = self.data.qpos.ravel()
