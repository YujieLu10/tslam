...
is_downstream_task = True
# from chamfer_distance import ChamferDistance
# chamfer_dist = ChamferDistance()

ADD_BONUS_REWARDS = True

class AdroitEnvV6(mujoco_env.MujocoEnv, utils.EzPickle):
    def __init__(self, 
    ...
    def step(self, a):
        ...
        if self.pretrain_mode == "pretrain":
            # pretrained on shape prediction task (tactile slam)
        ...
        if is_downstream_task:
            obj_pos  = obj_init_xpos#self.data.body_xpos[self.obj_bid].ravel()
            grasp_palm_pos = self.data.site_xpos[self.S_grasp_sid].ravel()
            # target_pos = self.data.site_xpos[self.target_obj_sid].ravel()

            reward = -0.1*np.linalg.norm(grasp_palm_pos-obj_pos)              # take hand to object
            if obj_pos[2] > 0.04:                                       # if object off the table
                reward += 1.0                                           # bonus for lifting the object
                # reward += -0.5*np.linalg.norm(grasp_palm_pos-target_pos)      # make hand go to target
            ...
            goal_achieved = True if obj_pos[2] > 0.06 else False
    ...
    def reset_model(self, num_traj_idx):
        ...
        if is_downstream_task:
            self.touch_obj_bid = self.sim.model.body_name2id('ObjectGrasp')
        else:
